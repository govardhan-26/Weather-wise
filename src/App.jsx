...
function App() {
  ...
  const fetchCityName = async (latitude, longitude) => {
    const apiKey = '3db24cc462794e499622e4c432192a7f';  // Replace with your OpenCage API key
    const url = `https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=${apiKey}`;
    try {
      const response = await axios.get(url);
      const city = response.data.results[0].components.county || response.data.results[0].components.state_district;
  ...
  const fetchWeather = async (latitude, longitude, label = 'Your Location') => {
    setIsLoading(true);
  ...
  const searchChangeHandler = async (enteredData) => {
    ...
    fetchWeather(latitude, longitude, enteredData.label);
  };
